...
if ! beeline -u jdbc:hive2://hadoop-03.uni.innopolis.ru:10001 -n team22 -p "$password" -f $base_path/sql/create_db.hql \
  --silent=true --outputformat=table > $base_path/output/temp_hive_results.txt 2> /dev/null; then
...
for i in "${queries[@]}"; do
    ...
    if ! beeline -u jdbc:hive2://hadoop-03.uni.innopolis.ru:10001 -n team22 -p "$password" -f $base_path/sql/q"$i".hql \
      --hiveconf hive.resultset.use.unique.column.names=false > $base_path/output/q"$i".csv 2>/dev/null; then
